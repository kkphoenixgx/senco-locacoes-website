...
export class VendasService {
  ...
  /** * Busca uma única venda pelo seu ID. * @param id O ID da venda. */
  ...
  /** * Cria uma nova venda. * @param vendaData Os dados da venda, incluindo o ID do cliente  itens. */
  ...
  /**
   * Atualiza o status de efetivação de uma venda.
   * @param id O ID da venda.
   * @param efetivada O novo status.
   */
  updateStatus(id: number, efetivada: boolean): Observable<Venda> {
    return this.http.put<any>(`${this.vendasUrl}/${id}/status`, { efetivada }).pipe(
      map(item => this.mapToVenda(item))
    );
  }
  ...
  deleteVenda(id: number): Observable<void> {
    return this.http.delete<void>(`${this.vendasUrl}/${id}`);
  }
  ...
  private mapToVenda(item: any): Venda {
    ...
    return new Venda(item.id, veiculos, new Date(item.dataVenda), item.precoTotal, item.clienteId, cliente, item.efetivada);
  }
}